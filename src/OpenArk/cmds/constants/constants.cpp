...
bool ReadConsoleOutput(const std::string& Cmdline, std::string& Output, DWORD& ExitCode, DWORD Timeout)
{
	...
	HANDLE ReadHd = NULL;
	HANDLE WriteHd = NULL;
	BOOL Ret = CreatePipe(&ReadHd, &WriteHd, &Sa, 64 * 0x1000);
	if (!Ret) {
	...
	Si.hStdOutput = WriteHd;
	Si.hStdError = WriteHd;
	Ret = CreateProcessA(NULL, (LPSTR)Cmdline.c_str(), NULL, NULL,
		TRUE, 0, NULL, NULL, &Si, &Pi);
	CloseHandle(WriteHd);
	if (!Ret) {
		CloseHandle(ReadHd);
		ERR("CreateProcessA %s failed, err:%d", Cmdline.c_str(), GetLastError());
		return false;
	}
	...
	while (1) {
		DWORD ReadLen = 0;
		if (ReadFile(ReadHd, Buffer, BlockSize, &ReadLen, NULL)) {
